...
public class AuctionHandler extends BaseHandler {
 ...
 private static final ConcurrentHashMap<Integer, Auction> AUCTION_CACHE =
 ...
 public static void evictFromCache(int auctionId) {
  ...
  private void handleCreate(HttpExchange ex) throws Exception {
    ...
    String desc = body.has("desc") && !body.get("desc").isJsonNull()
        ? body.get("desc").getAsString() : "";
    double price = body.get("price").getAsDouble();
    String imageFileName = body.has("imageFileName") && !body.get("imageFileName").isJsonNull()
        ? body.get("imageFileName").getAsString() : null;
  ...
  private void handleAutoBid(HttpExchange ex) throws Exception {
    ...
    // Neu bid vua dat len gia moi, phat thong bao toi tat ca client dang mo
