 ...
 *
 * CssType xác định bộ CSS dark nào dùng cho màn hình đó:
 *   MAIN    → styles.css
 *   LOGIN   → loginsignupstyles.css
 *   ADMIN   → adminstyles.css
 *   DETAIL  → listingpagestyles.css
 *   PROFILE → profilepagestyles.css
 *
...
public class ThemeManager {
    ...
    /**
     * Enum xác định loại CSS dark tương ứng với từng màn hình.
     * Light mode luôn dùng lightstyles.css bất kể CssType.
     */
    public enum CssType {
    ...
    /**
     * Tìm file CSS trong classpath bằng 3 chiến lược.
     * Giống resolveUrl() trong NavigationUtil.
     */
    private String resolveCss(String cssFileName) {
        ...
        url = ThemeManager.class.getResource(
                "/com/example/auctionmanagementsystem/css/" + cssFileName);
        if (url != null) return url.toExternalForm();

        // Chiến lược 3: ClassLoader — fallback cuối cùng
        url = ThemeManager.class.getClassLoader().getResource(
                "com/example/auctionmanagementsystem/css/" + cssFileName);
        if (url != null) return url.toExternalForm();
     ...
     * [HOẠT ĐỘNG ĐẦY ĐỦ ✅]
     *
     * Toggle Dark/Light mode, áp dụng lên Scene với CssType.MAIN.
     * Gọi từ themeButton trong AuctionListController.
     */
     ...
     *
     * Dark mode  → CSS riêng theo từng màn hình
     * Light mode → lightstyles.css cho mọi màn hình
     *
     * Gọi trong NavigationUtil.goTo() và openPopup()
     * để theme nhất quán trên mọi màn hình.
     *
    ...
    /**
     * Overload không cần CssType — mặc định dùng MAIN.
     * Dùng khi không biết loại màn hình.
     */
