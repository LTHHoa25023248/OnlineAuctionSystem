 ...
 *
 * Cung cấp 2 kiểu chuyển màn hình: 1. goTo() → thay thế toàn bộ scene hiện tại (chuyển trang) 2.
 * openPopup() → mở cửa sổ popup modal
 *
 * Mỗi màn hình tự động nhận đúng CssType để ThemeManager áp dụng stylesheet phù hợp.
 */
public class NavigationUtil {
  ...
  /**
   * Chuyển màn hình trực tiếp trên Stage. Tự xác định CssType từ fxmlPath để áp đúng stylesheet.
  ...
  /**
   * Mở popup modal, áp dụng theme với CssType tương ứng. Popup cũng nhận đúng theme như màn hình
   * chính.
   *
   * Dùng StageStyle.UNDECORATED — FXML root phải có background color đặc (bọc trong StackPane có
   * -fx-background-color).
   *
   ...
   *
   * Mapping: login / signup / forgot → LOGIN → loginsignupstyles.css Admin → ADMIN →
   * adminstyles.css detail → DETAIL → listingpagestyles.css profile → PROFILE →
   * profilepagestyles.css còn lại → MAIN → styles.css
   */
  private static ThemeManager.CssType getCssType(String fxmlPath) {
    if (fxmlPath == null)
      return ThemeManager.CssType.MAIN;
    if (fxmlPath.contains("login") || fxmlPath.contains("signup") || fxmlPath.contains("forgot"))
      return ThemeManager.CssType.LOGIN;
    if (fxmlPath.contains("Admin"))
      return ThemeManager.CssType.ADMIN;
    if (fxmlPath.contains("detail"))
      return ThemeManager.CssType.DETAIL;
    if (fxmlPath.contains("profile"))
      return ThemeManager.CssType.PROFILE;
    return ThemeManager.CssType.MAIN;
  }
   ...
   *
   * Chiến lược 1: Relative path từ vị trí class NavigationUtil Chiến lược 2: Absolute path từ gốc
   * classpath Chiến lược 3: ClassLoader (dùng khi module system can thiệp)
   */
  private static URL resolveUrl(String fxmlPath) {
    ...
    url = NavigationUtil.class.getResource("/com/example/auctionmanagementsystem/" + fxmlPath);
    if (url != null)
      return url;

    url = NavigationUtil.class.getClassLoader()
        .getResource("com/example/auctionmanagementsystem/" + fxmlPath);
    if (url != null)
      return url;
