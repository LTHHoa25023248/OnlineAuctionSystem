 ...
 *
 * Mở từ AuctionListController.openAuctionDetail(id).
 * Sau khi popup load xong, caller gọi loadAuction(id) để điền dữ liệu.
 *
 * Tính năng:
 *   - Hiển thị thông tin đầy đủ của auction (ảnh, giá, thời gian, mô tả)
 *   - Đặt giá (Place Bid)
 *   - Kết thúc đấu giá sớm (End Now — chỉ admin)
 *   - Đăng comment
 *
 * TODO: Thay dữ liệu mẫu trong loadAuction() bằng AuctionDAO.getById(id).
 *       Kết nối BidDAO.placeBid() và CommentDAO.post().
 */
public class AuctionDetailController {
    ...
    @FXML private Label         activeLabel;      // badge "Active"
    @FXML private Label         popularNowLabel;  // badge "Popular Now"
    @FXML private Label         userLabel;        // người đăng auction
    @FXML private Label         descriptionLabel;
    @FXML private Label         winnerLabel;      // hiện sau khi kết thúc
    @FXML private Label         bidValidationLabel; // lỗi / thành công khi bid

    // ── FXML fields — bid section ─────────────────────────────────────────────
    @FXML private MFXTextField  bidField;
    @FXML private MFXButton     placeBidButton;
    @FXML private MFXButton     endnowButton;  // chỉ hiện với admin

    // ── FXML fields — comment section ────────────────────────────────────────
    @FXML private MFXTextField  commentField;
    @FXML private MFXButton     commentPostButton;
    @FXML private MFXScrollPane commentsPane; // chứa danh sách comment
    ...
    /**
     * Điền dữ liệu auction vào UI.
     * Được gọi bởi AuctionListController sau khi popup load xong.
     *
    ...
    public void loadAuction(int auctionId) {
        ...
        descriptionLabel.setText(
                "This is a premium auction item. Bidding is open to all registered members.");
    ...
    /**
     * Đặt giá đấu.
     * Validate số tiền nhập vào, hiển thị thông báo thành công/lỗi.
     * TODO: Gọi BidDAO.placeBid(auctionId, userId, amount) khi có DB.
     */
    ...
    /**
     * Kết thúc đấu giá ngay lập tức (chỉ admin).
     * TODO: Gọi AuctionDAO.endNow(auctionId) và xác định winner.
     */
    ...
    /**
     * Đăng comment mới.
     * Thêm vào list tạm và reload danh sách comment.
     * TODO: Gọi CommentDAO.post() thay vì lưu trong memory.
     */
    ...
    /**
     * Render lại danh sách comment vào commentsPane.
     * Load comment_card.fxml cho mỗi comment và set vào VBox.
     */
