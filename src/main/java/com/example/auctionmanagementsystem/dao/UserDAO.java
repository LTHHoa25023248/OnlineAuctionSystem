 ...
 *   - register()          : Đăng ký tài khoản Bidder mới
 *   - registerSeller()    : Đăng ký tài khoản Seller mới (thêm storeName)
 *   - emailExists()       : Kiểm tra email đã tồn tại chưa
 *   - usernameExists()    : Kiểm tra username đã tồn tại chưa
 *   - saveResetCode()     : Lưu mã OTP reset password vào DB
 *   - verifyResetCode()   : Kiểm tra mã OTP còn hợp lệ không
 *   - resetPassword()     : Cập nhật mật khẩu mới sau khi xác thực OTP
 *   - selectById()        : Lấy User theo ID
...
public class UserDAO {
    ...
    public static boolean registerSeller(String firstName, String lastName,
                                         String username, String email,
                                         String phone, String password,
                                         String address) {
        if (usernameExists(username)) return false;
    ...
    /**
     * Map một ResultSet row thành đúng loại User (Bidder / Seller / Admin).
     * Gọi khi rs.next() đã trả về true.
     */
