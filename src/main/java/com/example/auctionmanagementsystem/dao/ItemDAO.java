     ...
     public Item selectById(int id) throws SQLException {
         try(Connection connect =new DatabaseConnection().getConnection()){
             ...
             if ("VEHICLE".equals(type)){
                 PreparedStatement ps2=connect.prepareStatement("SELECT * FROM vehicle_items WHERE item_id=?");
                 ps2.setInt(1, itemId);
                 ResultSet rs2=ps2.executeQuery();
                 if (rs2.next()){
                     Vehicle vehicle=new Vehicle(name, description, starting_price, rs2.getInt("year"), rs2.getDouble("mileage"));
                ...
                ResultSet rs2=ps2.executeQuery();
                if(rs2.next()){
                    Art art=new Art(name, description, starting_price, rs2.getString("artist"), rs2.getString("theme"), rs2.getString("material"));
                    art.setId(itemId);
                    return art;
                }
             }
            else if("ELECTRONICS".equals(type)){
                PreparedStatement ps2=connect.prepareStatement("SELECT * FROM electronics_items WHERE item_id=?");
                ps2.setInt(1,itemId);
                ResultSet rs2=ps2.executeQuery();
                if (rs2.next()){
                    Electronics electronic=new Electronics(name,description,starting_price, rs2.getString("brand"),rs2.getInt("warranty_months") );
